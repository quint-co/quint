...
Create a `bank.qnt` file and add the contents above to it. Then, let's try to check the invariant `no_negatives`. This invariant states that none of the balances should be less than zero, ever. An invariant is something that needs to be true for all reachable states.
