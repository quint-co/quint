...
<Tabs items={['NPM', 'Brew', 'Nix', 'Binaries']}>
 ...
 <Tabs.Tab>
 You can install Quint directly with [Homebrew](https://brew.sh/):
...
<Tabs items={['VSCode', 'Emacs', 'Vim', 'Helix', 'Zed']}>
 <Tabs.Tab>
...
```
 ...
 Install the [Quint extension](https://github.com/informalsystems/quint/tree/main/editor-plugins/zed) as a dev extension (it isn't published to Zed's extension registry yet):
 1. Clone the [Quint git repository](https://github.com/informalsystems/quint).
 2. Open the command palette and run `zed: install dev extension`.
 3. Select the `editor-plugins/zed` directory from your clone.

 Zed builds the grammar and starts `quint-language-server` automatically, installing it via npm if it isn't already on your `$PATH`.
